...
head(gold_ts_post2010, n=3)
# class of the data
class(gold_ts_post2010)
...
```{r time series plot}
plot_time_series(gold_ts_post2010, "Time Series Plot of Monthly Gold Price (2010-2025)", "ACF plot", "PACF plot")
```
...
plot(y=gold_ts_post2010,x=zlag(gold_ts_post2010),ylab='Current Price (in US$)', xlab='Previous Price (in US$)', main = "Scatter plot of in consecutive months", col="blue")

y = gold_ts_post2010             # Read the annual data into y
x = zlag(gold_ts_post2010)       # Generate first lag of the annual series
...
# Q-Q Plot
plot_qq(gold_ts_post2010, "Q-Q plot")
# Shapiro-Wilk test
...
stationarity_tests(gold_ts_post2010)
```
...
```{r Log Difference transformation}
gold_ts_post2010_log <- log(gold_ts_post2010)
gold_ts_post2010_logdiff <- diff(gold_ts_post2010, differences=1)
plot_time_series(gold_ts_post2010_logdiff,
                 "Time series plot of Log Difference Gold Price",
                 "ACF plot of BC Gold Price",
                 "PACF plot of BC Gold Price")

plot_qq(gold_ts_post2010_logdiff, "QQ plot after Box-Cox transformation")

shapiro.test(gold_ts_post2010_logdiff) # W = 0.99327, p-value = 0.8783
```
...
png("LogDiff_gold_ts_post2010.png", width = 1200, height = 600)
...
```{r}
McLeod.Li.test(y=gold_ts_post2010_logdiff,main="P-values of McLeod-Li test statistics 
...
par(mfrow=c(1,2))
acf(gold_ts_post2010_logdiff, main="ACF of the box cox series.")
pacf(gold_ts_post2010_logdiff, main="PACF of the box cox series.")
par(mfrow=c(1,1))
# from the ACF and PACF i get possible models as:
# ARIMA(p,d,q): ARIMA (2,0,2).

eacf(gold_ts_post2010_logdiff, ar.max = 10, ma.max = 10)
# from EACF
# ARIMA (p, d, q): ARIMA (0,0,5), ARIMA (0,0,6), ARIMA (1,0,2), ARIMA (1,0,3) 

# from the BIC plot
res = armasubsets(y=gold_ts_post2010_logdiff,nar=10,nma=10,y.name='p',ar.method='ols')
plot(res)
...
# ARIMA (1,0,2), ARIMA (5,0,7)}

model_507_ML <- arima(gold_ts_post2010_logdiff, order = c(5,0,7), method='ML')
coeftest(model_507_ML)

model_507_CSS <- arima(gold_ts_post2010_logdiff, order = c(5,0,7), method='CSS')
coeftest(model_507_CSS)

model_507_CSS_ML <- arima(gold_ts_post2010_logdiff, order = c(5,0,7), method='CSS-ML')
coeftest(model_507_CSS_ML)
```
```{r Parameter Estimation}

# ARIMA (1,0,2), ARIMA (5,0,7)}

model_102_ML <- arima(gold_ts_post2010_logdiff, order = c(1,0,2), method='ML')
coeftest(model_102_ML)

model_102_CSS <- arima(gold_ts_post2010_logdiff, order = c(1,0,2), method='CSS')
coeftest(model_102_CSS)

model_102_CSS_ML <- arima(gold_ts_post2010_logdiff, order = c(1,0,2), method='CSS-ML')
coeftest(model_102_CSS_ML)
```
...
```{r fig.height=10}
model_102 <- arima(gold_ts_post2010_logdiff, order = c(1,0,2), method='CSS-ML')
coeftest(model_102)
...
abs.r.res.gold_ts_post2010_logdiff = abs(rstandard(model_102))
sq.r.res.gold_ts_post2010_logdiff = rstandard(model_102)^2
```
...
par(mfrow=c(1,2))
acf(abs.r.res.gold_ts_post2010_logdiff, main="ACF of the absolute Log Difference series.")
pacf(abs.r.res.gold_ts_post2010_logdiff, main="PACF of the absolute Log Difference series.")
par(mfrow=c(1,1))
# Max(p,q) = 2, q = 2 => Max(p, q=2) = 2, p=0,1,2
# GARCH(0,2) GARCH(1,2) GARCH(1,2) 

eacf(abs.r.res.gold_ts_post2010_logdiff, ar.max = 10, ma.max = 10)
# max(p, q) = 0, q = 2 ==> max(p, q = 2) = 0 => No models
...
# max(p,q) = 2 and q = 1 ==> max(p, q = 1) = 2; hence p = 2 and GARCH(2,1).
# max(p,q) = 2 and q = 2 ==> max(p, q = 2) = 2; hence p = 0, 1, 2 and GARCH(0,2),GARCH(1,2),GARCH(2,2).
# max(p,q) = 3 and q = 1 ==> max(p, q = 1) = 3; hence p = 3 and GARCH(3,1).
# max(p,q) = 3 and q = 2 ==> max(p, q = 2) = 3; hence p = 3 and GARCH(3,2).

# GARCH(0,1),GARCH(1,1), GARCH(2,1), GARCH(0,2),GARCH(1,2),GARCH(2,2), GARCH(3,1), GARCH(3,2)

# from the BIC plot
res = armasubsets(y=abs.r.res.gold_ts_post2010_logdiff,nar=10,nma=10,y.name='p',ar.method='ols')
plot(res)
# max(p, q) = 5, q = 2 ==> max(p, q = 2) = 5; hence p = 5 and GARCH(5,2)
# max(p, q) = 7, q = 2 ==> max(p, q = 2) = 7; hence p = 7 and GARCH(7,2)
...
par(mfrow=c(1,2))
acf(sq.r.res.gold_ts_post2010_logdiff, main="ACF of the squared box cox series.")
pacf(sq.r.res.gold_ts_post2010_logdiff, main="PACF of the squared box cox series.")
par(mfrow=c(1,1))
# Max(p,q) = 2, q = 2 => Max(p, q=2) = 2, p=0,1,2
# GARCH(0,2) GARCH(1,2) GARCH(1,2) 

eacf(sq.r.res.gold_ts_post2010_logdiff, ar.max = 10, ma.max = 10)
# max(p,q) = 0 and q = 2 ==> max(p, q = 2) = 0; Does not lead to any models.
# max(p,q) = 0 and q = 3 ==> max(p, q = 3) = 0; Does not lead to any models.
# max(p,q) = 1 and q = 2 ==> max(p, q = 2) = 1; Does not lead to any models.
# max(p,q) = 1 and q = 3 ==> max(p, q = 3) = 1; Does not lead to any models.
# max(p,q) = 2 and q = 2 ==> max(p, q = 2) = 2; hence p = 0, 1, 2 and GARCH(0,2),GARCH(1,2),GARCH(2,2).
# max(p,q) = 3 and q = 2 ==> max(p, q = 2) = 3; hence p = 3 and GARCH(3,2).

# GARCH(0,2), GARCH(1,2), GARCH(2,2), GARCH(3,2)

# from the BIC plot
res = armasubsets(y=sq.r.res.gold_ts_post2010_logdiff,nar=10,nma=10,y.name='p',ar.method='ols')
plot(res)
# max(p,q) = 9 and q = 2 ==> max(p, q = 2) = 9; hence p = 9 and GARCH(9,2)
# max(p,q) = 9 and q = 7 ==> max(p, q = 7) = 9; hence p = 9 and GARCH(9,7)
...
# GARCH(0,1), GARCH(0,2),GARCH(1,1), GARCH(1,2), GARCH(2,1), GARCH(2,2), GARCH(3,1), GARCH(3,2), GARCH(5,2), GARCH(7,2), GARCH(9,2), GARCH(9,7)

```{r fig.height=10}
m.01 = garch(gold_ts_post2010_logdiff, order=c(0,1), trace=FALSE)
summary(m.01)
...
```{r fig.height=10}
m.11 = garch(gold_ts_post2010_logdiff, order=c(1,1), trace=FALSE)
summary(m.11)
...
```{r fig.height=10}
model_102_01 <- fGarch::garchFit(~ arma(1,2)+garch(1,0), data = gold_ts_post2010_logdiff, trace=F)
summary(model_102_01)
residual.analysis(model = model_102_01, std = TRUE,start = 2, class = "fGARCH")
```
...
```{r fig.height=10}
model_102_11 <- fGarch::garchFit(~ arma(1,2)+garch(1,1), data = gold_ts_post2010_logdiff, trace=F)
# Warning: NaNs produced
summary(model_102_11)
residual.analysis(model = model_102_11, std = TRUE,start = 2, class = "fGARCH")
```
...
df <- data.frame(AIC = c(model_102_01@fit$ics[1],model_102_11@fit$ics[1]),
BIC = c(model_102_01@fit$ics[2],model_102_11@fit$ics[2]))
rownames(df) <- c("ARMA(1,2)+GARCH(0,1)","ARMA(1,2)+GARCH(1,1)")
df
```
### ------------> OBSERVATION:
...
model_102_01_2 <- ugarchfit(spec = spec, data = gold_ts_post2010_logdiff,
solver = "hybrid",
...
arma_orders <- list(c(1, 2), c(1, 2))
garch_orders <- list(c(0, 1), c(1, 1))

# Call the function with your data and the list of ARMA+GARCH models
accuracy_results <- fit_armagarch_models(gold_ts_post2010_logdiff, arma_orders, garch_orders)
...
# 13. Forecasting
```{r}
frc <- ugarchforecast(model_102_01_2,n.ahead=10,data=gold_ts_post2010_logdiff)
frc
plot(frc, which = 1)
```
...
```{r}
plot(gold_ts_post2010, xlim= c(2008.7,2018.4), ylim = c(0,400),
